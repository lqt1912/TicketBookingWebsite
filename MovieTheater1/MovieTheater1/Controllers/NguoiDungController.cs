    ...
    public class NguoiDungController : Controller
    {
        ...
        public ActionResult DangKi(FormCollection collection, string Matkhau, THANHVIEN kh)
        {
            ...
            else if (this.IsCaptchaValid(""))
            {
                ...
                new MailHelper().SendMail(kh.EMAIL, "Thông tin tài khoản của quý khách tại hệ thống QKT Cinema", content);
        ...
        public ActionResult DoiMatKhau(FormCollection collection)
        {
            ...
            {
                ViewData["Loi3"] = "Pass không giống ";
            }
            ...
            else
            {
                ...
                {
                    ViewBag.Thongbao = "Nhập sai pass mà đổi cái gì";
                }
            }
        ...
        [HttpPost]
        public ActionResult EmailHoTro(FormCollection f)
        {
            string content = System.IO.File.ReadAllText(Server.MapPath("~/Mail-Support.html"));

            content = content.Replace("{{khachhang}}", f["sender"]);
            content = content.Replace("{{email}}", f["email"]);
            content = content.Replace("{{sodienthoai}}",f["sdt"]);
            content = content.Replace("{{message}}", f["message"]);

            var toEmail = ConfigurationManager.AppSettings["ToEmailAddress"].ToString();

            new MailHelper().SendMail(toEmail, "Thông tin phản hồi ", content);
            Session["SupportEmail"] = "Đã gửi email hỗ trợ ";
            return RedirectToAction("TrangChu", "QKTCinema");

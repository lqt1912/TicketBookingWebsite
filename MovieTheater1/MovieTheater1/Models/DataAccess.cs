	...
	public class DataAccess
	{
		...
		public static List<PHIM> GetPhimDangChieuShort(int number)
		{
			...
			using (var db = getEntities())
			{
				...
				var result2 = db.PHIMs.SqlQuery("SELECT top 6 * FROM PHIM Where CURRENT_TIMESTAMP <= THOIGIANKT ORDER BY  CURRENT_TIMESTAMP - THOIGIANBD");
		...
		public static List<PHIM> GetPhimDangChieuAll(int number)
		{
			...
			{
				var now = DateTime.Now;
				var result2 = db.PHIMs.SqlQuery("SELECT * FROM PHIM Where CURRENT_TIMESTAMP <= THOIGIANKT ");
				return result2.ToList();
			}
		}
